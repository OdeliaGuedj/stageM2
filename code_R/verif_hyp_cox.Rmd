...
#### 1.1.1.1 Time on study as time scale
...
for(var in test_univar_deces){
  ...
  cox_std = coxph(Surv(time_to_deces,deces)~data2_std[,var], data = data2_std)
...
plot (survfit(Surv(time_to_deces, deces) ~ plaques, data = data2_std), fun = "cloglog")
```

#### 1.1.1.2 Age as time scale

Plot des résidus de schonfeld 
```{r}
g = c()
for(var in test_univar_deces){
  cox = coxph(Surv(age_at_deces,deces)~data2[,var], data = data2)
  cox_std = coxph(Surv(age_at_deces,deces)~data2_std[,var], data = data2_std)
  schon.test = cox.zph(cox)
  schon.test_std = cox.zph(cox_std)
  g = c(g,ggcoxzph(schon.test) + 
          ylab(" ") + 
...
```{r}
plot (survfit(Surv(age_at_deces, deces) ~ plaques, data = data2), fun = "cloglog")
plot (survfit(Surv(age_at_deces, deces) ~ plaques, data = data2_std), fun = "cloglog")
```

### 1.1.2 Avec le CHD comme outcome

#### 1.1.2.1 Time on study as time scale
```{r}
g = c()
for(var in test_univar_chd){
  cox = coxph(Surv(time_to_chd,chd)~data2[,var], data = data2)
  cox_std = coxph(Surv(time_to_chd,chd)~data2_std[,var], data = data2_std)
  schon.test = cox.zph(cox)
  schon.test_std = cox.zph(cox_std)
  g = c(g,ggcoxzph(schon.test) + 
          ylab(" ") + 
...
plot (survfit(Surv(time_to_chd, chd) ~ plaques, data = data2_std), fun = "cloglog")
```
#### 1.1.2.2 Age as time scale
```{r}
g = c()
for(var in test_univar_chd){
  cox = coxph(Surv(age_at_chd,chd)~data2[,var], data = data2)
  cox_std = coxph(Surv(age_at_chd,chd)~data2_std[,var], data = data2_std)
  schon.test = cox.zph(cox)
  schon.test_std = cox.zph(cox_std)
  g = c(g,ggcoxzph(schon.test) + 
          ylab(" ") + 
...
```{r}
plot (survfit(Surv(age_at_chd, chd) ~ plaques, data = data2), fun = "cloglog")
plot (survfit(Surv(age_at_chd, chd) ~ plaques, data = data2_std), fun = "cloglog")
```
...
for(var in test_univar_deces){
  if(is.numeric(data2[,var])){
    ...
    out_idx = cut_outlier_quantile(var,data2,0.001)$out_idx

    tmp_tab = verifHyp_loglin_coxUnivar(var,"time_to_deces","deces",data2,nb.class = 6, method = "center")
