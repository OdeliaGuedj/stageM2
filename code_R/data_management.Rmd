...
var_ajust = c("age0","sexe" , "qm16" , "qm10" , "diab" , "bio10" , "bmi" , "mbp", "ldl","trb17", "year_naiss_cat")
...
# 12. Création d'une base de donnée sans les NA dans au moins une des variables d'echotracking_clean

```{r}
data4 = data2[complete.cases(data2[,echotracking_clean]),]
```
```{r}
data4_std = data4
...
```{r}
data5 = data2[complete.cases(data2[,c(var_ajust,echotracking_clean)]),]
```
```{r}
data5_std = data5
for(var in colnames(data5)){
  if(!(var%in%no_scale)){
    data5_std[,var] = (data5[,var]-mean(data5[,var], na.rm = T))/sd(data5[,var], na.rm = T)
  }
}
rm(var)
```
...
for(var in colnames(data5)){
  if(!(var%in%c(no_scale,"age0"))){
    data5_std2[,var] = (data5[,var]-mean(data5[,var], na.rm = T))/sd(data5[,var], na.rm = T)
  }
}
rm(var)
```
