...
```{r}
data$pa_cat = NA
data$pa_cat[data$pas_basale__mm_hg_ < 120 & data$pad_basale__mm_hg_ < 80] = "Normal"
data$pa_cat[data$pas_basale__mm_hg_< 129 & data$pas_basale__mm_hg_ > 120 & data$pad_basale__mm_hg_ < 80] = "Elevée"
data$pa_cat[(data$pas_basale__mm_hg_ < 139 & data$pas_basale__mm_hg_ > 130) | (data$pad_basale__mm_hg_ < 89 & data$pad_basale__mm_hg_ > 80)] = "HP Stage 1"
data$pa_cat[data$pas_basale__mm_hg_ >= 140 | data$pad_basale__mm_hg_ >= 90] = "HP Stage 2"
data$pa_cat[data$pas_basale__mm_hg_ >= 180 | data$pad_basale__mm_hg_ > 120] = "HP Crisis"
```
```{r}
table(data$pa_cat, useNA = "ifany")
```
```{r}
data$mbp_cat = quant.cut(data$mbp, 3)
table(data$mbp_cat)
```
...
bio = c("trb17", "ldl","bio10","mbp","bio6", "pa_cat", "mbp_cat")
...
echotracking_clean =  c("pas_basale__mm_hg_","pad_basale__mm_hg_", "fc_basale","diam_tre_en_bm__mm_","imt___m_","diam_tre_en_fbm__mm_","distension___m_","pwvloc", "dploc","dist_rate_moy","rr_int_moy","lf_dist_rate_t","hf_dist_rate_t","hf_dist_rate_c","lf_rrint_t","hf_rr_int_t","hf_rrint_c","freq_resp","lf_coh_moy","lf_gain_moy","lf_phase","hf_c_coh_moy","hf_c_gain_moy","hf_c_phase","hf_t_coh_moy","hf_t_gain_moy","hf_t_phase","dist_moy","cws","coeffDist","compli","wcsa","pr_sence_de_plaque_s____l__cho","logain","logyoung","dextdias","stiffness")

var_et_jpe_tot = c("diam_tre_en_bm__mm_", "imt___m_", "distension___m_", "dploc", "dist_rate_moy",  "rr_int_moy","lf_dist_rate_t","hf_dist_rate_t","lf_rrint_t","hf_rr_int_t","lf_coh_moy", "hf_c_coh_moy", "hf_c_gain_moy", "cws","coeffDist", "compli", "wcsa", "young","logain","stiffness")

var_et_jpe_inf = c("diam_tre_en_bm__mm_" , "imt___m_", "distension___m_", "dploc", "dist_rate_moy","coeffDist", "young","logain","stiffness")
```
...
# 8. Création base de données sans les prévalents cvd
