...
## 1.4 Fonction cox_multivar

Permet de fitter des régression de cox multivariée et de récupérer les inf du dernier coefficients qui correspond au paramètre d'échotracking que l'on étudie.
...
cox_multivar = function(formula, list_param,dt, round = T, d.round = 2){
  ...
  df_res = data.frame()
    ...
    for(param in list_param){
      ...
      if(round == T){
          df_tmp = data.frame("Names" = rownames(summary$coefficients)[r],
                    ...
                    "p.shonfeld" = signif(test.shonfeld$table[r,3],d.round+1),
                    "prop.hyp" = ifelse(test.shonfeld$table[r,3] < 0.05,"X"," "))
...
## 3.1 Les modèles des analyses déjà effectuées (uniquement à baseline) et sur les 9 paramètres carotidiens
