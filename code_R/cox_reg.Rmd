...
plot_pv_HR = function(cox_nice_info, hj = 0, vj = 0, main = " "){
  ...
  # Pour pouvoir utiliser cette fonction il faut que cox_nice_info soit un objet cox_univer ou cox_multivar utilisé avec l'argiment round = F
  
  ggplot(cox_nice_info, aes(cox_nice_info$p.value, cox_nice_info$HR)) + 
    ...
    geom_text_repel(aes(label = cox_nice_info$Name), box.padding   = 0.35, point.padding = 0.5, segment.color = "grey50", force = 15,segment.size  = 0.5 ,#epaisseur segments
                    size = 3 #taille label 
                    )+
    ...
    for(param in list_param){
      ...
      if(round == T){
          df_tmp = data.frame("Name" = rownames(summary$coefficients)[r],
                    ...
                    "p.shonfeld" = signif(test.shonfeld$table[r+1,3],d.round+1),
        ...
        df_tmp = data.frame("Name" = rownames(summary$coefficients)[r],
                    ...
                    "p.value" = signif(summary$coefficient[r,5],d.round+3),
                    "p.shonfeld.global" = signif(test.shonfeld$table[r+1,3],d.round+1))
