...
```
# 2. Création base de données centrées réduite
```{r}
all_var_num = c()
for(var in colnames(data)){
  if(is.numeric(data[,var]) & !(is.integer(data[,var]))){
    #print(var)
    all_var_num = c(all_var_num,var)
  }
}
#all_var_num
```
...
```{r}
data_std = data
data_std[,all_var_num] = scale(data_std[,all_var_num])
```
...
# 3. Fonctions pour modèles de Cox
...
for(evt in var_evt){
  ...
  for(var in param_carotid){
    ...
    coxSTD = cox_univar(time, var, evt, data_std)
...
for(evt in var_evt){
  ...
  for(var in colnames(data_echo)){
    ...
    coxSTD = cox_univar(time, var, evt, data_std, round = F)
...
```{r}
test = c(which(list_univar_et[["deces"]]$pv_signif == 1),
which(list_univar_et[["aomi"]]$pv_signif == 1),
which(list_univar_et[["tdr"]]$pv_signif == 1),
which(list_univar_et[["chd"]]$pv_signif == 1),
which(list_univar_et[["stroke"]]$pv_signif == 1))
```
```{r}
length(test)
length(unique(test))
length(test2)
test2 = test[-unique(test)]
test2
```
...
i = 1
POur chaque evt
  Pour chaque var d'et
    formula1 = formula(Surv(time_to_deces,deces) ~ var + age0 + sexe + qm16 + qm10, env = data)
    list[[i]] = formula1
    i = i+1
```{r}
#formula1 = list()
#i = 1
for(var in data[,param_carotid]){
  formula1 = update(formula1, . ~ . + var)
  print(formula1)
}
...
formula2 = "age0 + sexe + qm16 + qm10 + diab + bio10"
formula3 = "age0 + sexe + qm16 + qm10 + diab + bio10 + bmi"
formula4 = "age0 + sexe + qm16 + qm10 + diab + bio10 + bmi + mbp"
```
```{r}
formula1
```
